...
import { LedMatrix } from './led-matrix.jsx';
import AheuiJITWorker from '../lib/worker/aheui-jit.js?worker';
...
const output = computed(() => {
  const worker = new AheuiJITWorker();
  worker.postMessage(code.value);
  const ref: { resolved: boolean; value: any; promise: Promise<any> } = {
    resolved: false,
    value: null,
...
export function LedMatrixEdit(): JSX.Element {
  const { resolved, value, promise } = output.value;
  if (!resolved) {
    throw promise;
  }
